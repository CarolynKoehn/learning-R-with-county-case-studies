...
my_county_proj <- st_transform(_________,
                               crs = st_crs(___))
```
```{r}
#| include: false
...
avg_ag_quality <- ____(values(______), na.rm=TRUE)
```
```{r}
#| include: false
...
**\textcolor{orange}{(`r cont_count()`)}** Download vector data for land management from the shared Google Drive 
...
bear_lake_mgmt$Mgmt_Category <- ifelse(bear_lake_mgmt$AGNCY_NAME == "PRIVATE", "Private",
                            ifelse(bear_lake_mgmt$AGNCY_NAME %in% c("BIA", "IR"), "Tribal",
                              ifelse(bear_lake_mgmt$AGNCY_NAME %in% c("OTHER", "HSTRCWTR"), "Other",
                                ifelse(str_starts(bear_lake_mgmt$AGNCY_NAME, "STATE"), "State", "Federal"))))
```
...
mgmt_acres <- aggregate(_____ ~ ______, data = _______, FUN = ___)
```
```{r}
#| include: false

mgmt_acres <- aggregate(GIS_ACRES_ ~ Mgmt_Category, data = bear_lake_mgmt, FUN = sum)
```
...
**\textcolor{orange}{(`r cont_count()`)}** Add percent management data to the shared Google Sheet.
...
pvr <- rast("R:/SarahHalperin/Data/AFT_Data/productivity_versatility_resiliency_2016_conus_120m.tif")
pvr_crop <- crop(pvr, st_transform(idaho, crs(pvr)), mask=TRUE)
writeRaster(pvr_crop, "C:/Users/carolynkoehn/Desktop/Module 6/data/productivity_versatility_resiliency_2016_ID.tif")
```