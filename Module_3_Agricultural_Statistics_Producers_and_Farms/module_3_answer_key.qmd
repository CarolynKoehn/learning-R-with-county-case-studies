---
title: "Agricultural Statistics: Producers and Farms -- Answer key"
format: pdf
---
...
```{r}
print(my_county_producers[ ,c("Year", "County", "Data.Item", "Value")])
```
...
```{r}
total_producers <- subset(my_county_producers, 
                          Data.Item == "PRODUCERS, (ALL) - NUMBER OF PRODUCERS")

total_producers <- total_producers$Value
```
...
```{r}
percent_male <- male_producers / total_producers * 100
...
```{r}
female_producers <- subset(my_county_producers, 
                         str_detect(string = Data.Item,
                                    pattern = "FEMALE"))
female_producers <- female_producers$Value
percent_female <- female_producers / total_producers * 100

amin_producers <- subset(my_county_producers, 
                         str_detect(string = Data.Item,
                                    pattern = "AMERICAN INDIAN"))
amin_producers <- amin_producers$Value
percent_amin <- amin_producers / total_producers * 100

hispanic_producers <- subset(my_county_producers, 
                         str_detect(string = Data.Item,
                                    pattern = "HISPANIC"))
hispanic_producers <- hispanic_producers$Value
percent_hispanic <- hispanic_producers / total_producers * 100

white_producers <- subset(my_county_producers, 
                         str_detect(string = Data.Item,
                                    pattern = "WHITE"))
white_producers <- white_producers$Value
percent_white <- white_producers / total_producers * 100
```
...
```{r}
unique_commodities <- unique(my_county_commodities$Commodity)

print(length(unique_commodities))
```
...
# examples
colnames(size)
head(size[ ,c("Year", "County", "Data.Item", "Domain.Category", "Value")])
```
...
```{r}
bear_lake_farms <- subset(size, County == "BEAR LAKE" & Year == 2017)

print(bear_lake_farms[ ,c("County", "Domain", "Domain.Category", "Value")])
```
...
```{r}
class(bear_lake_farms$Value)

bear_lake_farms$Value <- str_remove_all(bear_lake_farms$Value, ",")
bear_lake_farms$Value <- as.numeric(bear_lake_farms$Value)
...
```{r}
total_farms <- subset(bear_lake_farms, Domain == "TOTAL")
total_farms <- total_farms$Value
...
```{r}
bear_lake_tenure <- subset(bear_lake_farms, Domain == "TENURE")

print(bear_lake_tenure[ ,c("Domain", "Domain.Category")])
```
...
**\textcolor{orange}{(`r cont_count()`)}** Create a new column called `Percent_farms_rounded` with the percentage of farms in each category rounded to 1 decimal place.

```{r}
my_county_tenure_agg$Percent_farms_rounded <- round(my_county_tenure_agg$Percent_farms, 1)
```


**\textcolor{orange}{(`r cont_count()`)}** Print the `my_county_tenure_agg` data frame and add the rounded values for each category to the Google sheet.
...
**\textcolor{orange}{(`r cont_count()`)}** Create a new object and use the data from step (`r size_data_ready`) to save a subset of the farm operations data where `Domain` is equal to `AREA OPERATED`. Print the `Domain` and `Domain.Category` columns of this subset to check that it worked.

```{r}
my_county_sizes <- subset(bear_lake_farms, Domain == "AREA OPERATED")

print(my_county_sizes[ ,c("Domain", "Domain.Category")])
```
...
```{r}
my_county_sizes_agg$Percent_farms <- 
  my_county_sizes_agg$Value / total_farms * 100
```
...
```{r}
my_county_sizes_agg$Percent_farms_rounded <- 
  round(my_county_sizes_agg$Percent_farms, 1)
