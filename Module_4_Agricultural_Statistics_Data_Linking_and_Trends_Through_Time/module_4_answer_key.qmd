---
title: "Agricultural Statistics: Data Linking and Trends through Time -- Answer key"
format: pdf
---
...
print(unique(my_county_size_tenure$Year))
print(unique(my_county_size_tenure$Domain))
```
...
```{r}
class(my_county_size_tenure$Value)

library(stringr)

my_county_size_tenure$Value <- str_remove_all(my_county_size_tenure$Value, ",")
my_county_size_tenure$Value <- as.numeric(my_county_size_tenure$Value)
...
```{r}
colnames(my_county_size_tenure_2017_selected)
colnames(my_county_size_tenure_1997_selected)
```
...
```{r}
tenure_change <- subset(my_county_size_tenure_joined, 
                        str_detect(Domain.Category, "TENURE"))

print(tenure_change[ ,c("Domain.Category", "Difference")])
```
...
```{r}
tenure_change$Percent_difference <- 
  tenure_change$Difference / tenure_change$Value_1997 * 100

print(tenure_change[ ,c("Domain.Category", "Percent_difference")])
```
...
```{r}
my_county_sizes_agg$Percent_difference <- 
  my_county_sizes_agg$Difference_sum / my_county_sizes_agg$Value_1997_sum * 100
...
print(unique(my_county_agland$Year))
print(unique(my_county_agland$Data.Item))
```
...
```{r}
class(my_county_agland$Value)

my_county_agland$Value <- str_remove_all(my_county_agland$Value, ",")
my_county_agland$Value <- as.numeric(my_county_agland$Value)
...
```{r}
my_county_land <- 
  subset(my_county_agland, str_detect(Data.Item, "LAND AREA, INCL NON-AG"))

print(my_county_land)
```
...
# summarise
my_county_land <- summarise(my_county_land, .by=Year, 
                            Value = sum(Value, na.rm=TRUE))

# aggregate
my_county_land <- aggregate(Value ~ Year, data = my_county_land, sum)
```
...
#summarise
my_county_agland <- summarise(my_county_agland, .by = Year,
                              Value = sum(Value, na.rm=TRUE))
```
```{r}
# aggregate
...
```{r}
my_county_land_join <- full_join(my_county_agland,
                                 my_county_land,
                                 by="Year",
                                 suffix = c("_ag", "_total"))
```
...
```{r}
my_county_land_join$Perc_ag <- 
  my_county_land_join$Value_ag / my_county_land_join$Value_total * 100
```
...
```{r}
my_county_land_join[5, "Perc_ag"] - my_county_land_join[1, "Perc_ag"]
```
...
```{r}
my_county_land_join$rate <- 
  my_county_land_join$Perc_ag - my_county_land_join$Perc_lag
```
...
```{r}
print(my_county_land_join[ ,c("Year", "rate")])
```
...
**\textcolor{orange}{(`r cont_count()`)}** Create a new code chunk and use `read.csv` to read in the data from `ag_sales_ID_1997-2017.csv` and `ag_expenses_ID_1997-2017.csv`. These datasets have information about total agricultural sales and expenses, respectively, for 1997, 2002, 2007, 2012, and 2017. 
...
**\textcolor{orange}{(`r cont_count()`)}** Subset each data frame to your county.
...
**\textcolor{orange}{(`r cont_count()`)}** Preview the data by printing the `head` of the columns `Year`, `Data.Item`, `Domain.Category`, and `Value` in each data frame. For more clarity, your can click the subset in the Environment tab to display the whole data frame.

```{r}
head(sales[ ,c("Year", "Data.Item", "Domain.Category", "Value")])
head(expenses[ ,c("Year", "Data.Item", "Domain.Category", "Value")])
```
...
sales$Value <- str_remove_all(sales$Value, ",")
sales$Value <- as.numeric(sales$Value)

class(sales$Value)

class(expenses$Value)

expenses$Value <- str_remove_all(expenses$Value, ",")
expenses$Value <- as.numeric(expenses$Value)
...
```{r}
# with summarise
sales_by_year <- summarise(sales, 
                           .by=Year, 
                           Value = sum(Value, na.rm=TRUE))
expenses_by_year <- summarise(expenses, 
                              .by=Year, 
                              Value = sum(Value, na.rm=TRUE))

# with aggregate
sales_by_year <- aggregate(Value ~ Year, data = sales, FUN = sum)
expenses_by_year <- aggregate(Value ~ Year, data = expenses, FUN = sum)
```
...
```{r}
profits <- full_join(sales_by_year, 
                     expenses_by_year, 
                     by="Year", 
                     suffix=c("_sales", "_expenses"))
...
```{r}
profits[5, "profit"] - profits[1, "profit"]
```
...
```{r}
profits$lagged <- lag(profits$profit, n=1)
```
...
**\textcolor{orange}{(`r cont_count()`)}** Print the columns `Year` and `rate`.

```{r}
print(profits[ ,c("Year", "rate")])
```


**\textcolor{orange}{(`r cont_count()`)}** Record the rate for 2017 on the shared Google sheet.
