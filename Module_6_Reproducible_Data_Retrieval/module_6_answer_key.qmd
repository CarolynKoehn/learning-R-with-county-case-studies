---
title: "Reproducible Data Retrieval: Pulling Data Directly into R - Answer Key"
format: pdf
---
...
**\textcolor{orange}{(`r cont_count()`)}** Import `tidycensus` library. 
...
```{r}
not_citizen <- subset(citizens, variable == "B05001_006")$estimate
tot_pop <- subset(citizens, variable == "B05001_001")$estimate

not_citizen / tot_pop * 100
```
...
```{r}
farms_w_eas_2017 <- subset(farms_with_conservation, year == 2017)$Value
farms_w_eas_1997 <- subset(farms_with_conservation, year == 1997)$Value

print(farms_w_eas_2017)
farms_w_eas_2017 - farms_w_eas_1997
```
...
```{r}
plot(x = farms_with_conservation$year, 
     y = farms_with_conservation$Value,
     type = "l")
```
