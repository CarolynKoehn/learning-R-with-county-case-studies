---
title: "Demographic Data - Answer Key"
format: pdf
---
...
**\textcolor{orange}{(`r cont_count()`)}** Download the following `.csv` files into your `data` folder from this [GitHub folder](https://github.com/CarolynKoehn/learning-R-with-county-case-studies/tree/main/Module_5_Demographic_Data/data): `ID_employment.csv` and `ID_county-to-county-2013-2017-ins-outs-nets-gross.csv`.
...
**\textcolor{orange}{(`r cont_count()`)}** Look at the `ID_employment_codes.csv` file in the [GitHub folder](https://github.com/CarolynKoehn/learning-R-with-county-case-studies/tree/main/Module_5_Demographic_Data/data).
...
|---|---|---|
| Agriculture | Agriculture, forestry, fishing and hunting  | C24030_004, **C24030_031** |
| Manufacturing/Warehousing | Manufacturing  | **C24030_007**, **C24030_034** |
| Manufacturing/Warehousing | Transportation and warehousing  | **C24030_011**, **C24030_038** |
| Hospitality | Arts, entertainment, and recreation, and accommodation and food services  | **C24030_024**, **C24030_051**  |
| Technology | Information  | **C24030_013**, **C24030_040**  |
| Technology | Professional, scientific, and technical services  | **C24030_018**, **C24030_045**  |
...
```{r}
ag_est <- sum(my_county_ag_employed$estimate)
```
...
```{r}
percent_ag <- (ag_est / total_population) * 100

print(round(percent_ag, 1))
``` 
...
```{r}
# manufacturing
my_county_man_employed <- subset(my_county_employ,
                                variable %in% c("C24030_007", "C24030_034",
                                                "C24030_011", "C24030_038"))
man_est <- sum(my_county_man_employed$estimate)
percent_man <- (man_est / total_population) * 100
print(round(percent_man, 1))

# hospitality
my_county_hosp_employed <- subset(my_county_employ,
                                variable %in% c("C24030_024", "C24030_051"))
hosp_est <- sum(my_county_hosp_employed$estimate)
percent_hosp <- (hosp_est / total_population) * 100
print(round(percent_hosp, 1))

# technology
my_county_tech_employed <- subset(my_county_employ,
                                variable %in% c("C24030_013", "C24030_040",
                                                "C24030_018", "C24030_045"))
tech_est <- sum(my_county_tech_employed$estimate)
percent_tech <- (tech_est / total_population) * 100
print(round(percent_tech, 1))
```
...
**\textcolor{orange}{(`r cont_count()`)}** Calculate the sum of all retirees in your county.

```{r}
num_retired <- sum(my_county_retire$estimate)
```


**\textcolor{orange}{(`r cont_count()`)}** Calculate the percent of county residents who are retirees over 65, round to one decimal place, and record in the shared Google sheet. 

```{r}
percent_retired <- (num_retired / total_population) * 100

print(round(percent_retired, 1))
```
...
class(my_county_migration$Flow.from.Geography.B.to.Geography.A)
class(my_county_migration$Counterflow.from.Geography.A.to.Geography.B1)

my_county_migration$Flow.from.Geography.B.to.Geography.A <- 
  str_remove_all(string = my_county_migration$Flow.from.Geography.B.to.Geography.A,
                 pattern = ",")
my_county_migration$Flow.from.Geography.B.to.Geography.A <- 
  as.numeric(my_county_migration$Flow.from.Geography.B.to.Geography.A)

my_county_migration$Counterflow.from.Geography.A.to.Geography.B1 <- 
  str_remove_all(string = my_county_migration$Counterflow.from.Geography.A.to.Geography.B1,
                 pattern = ",")
my_county_migration$Counterflow.from.Geography.A.to.Geography.B1 <- 
  as.numeric(my_county_migration$Counterflow.from.Geography.A.to.Geography.B1)
...
```{r}
NumEntering <- sum(my_county_migration$Flow.from.Geography.B.to.Geography.A,
                   na.rm = TRUE)
```
...
```{r}
NumLeaving <- sum(my_county_migration$Counterflow.from.Geography.A.to.Geography.B1, 
                  na.rm = TRUE)
```
