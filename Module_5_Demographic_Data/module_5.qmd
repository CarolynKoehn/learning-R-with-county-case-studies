...
Date Delivered: November 2023 (expected)

Outlet of Delivery: VIP team meeting
...
**\textcolor{orange}{(`r i`)}** Open RStudio by double-clicking the project file you created. **\textcolor{orange}{(`r cont_count()`)}** Create a new Quarto file (refer to the "Create a Quarto Document" section in the first module).

**\textcolor{orange}{(`r cont_count()`)}** Download the following `.csv` files into your `data` folder from the shared [Google Drive folder](https://drive.google.com/drive/folders/1QmtynAu6nZiitjeJCkCT9Sx3loq72_n1?usp=drive_link): ID_employment.csv and ID_county-to-county-2013-2017-ins-outs-nets-gross.csv.

### Employment metrics

**\textcolor{orange}{(`r cont_count()`)}** Create a new code chunk and use `read.csv` to read in the data from ID_employment.csv. **\textcolor{orange}{(`r cont_count()`)}** Use `head` and at least one other function from Module 1 to inspect the data.
...
This is 2017 data from the American Community Survey. It has information on the gender and employment sector of residents in each county, but this information is contained in codes in the variable column. The meanings of these codes are stored in ID_employment_codes.csv in the Google Drive folder. The `moe` column stands for "margin of error" -- an important estimate of uncertainty. However, we will not be using it for these case studies.
...
**\textcolor{orange}{(`r cont_count()`)}** Look at the ID_employment_codes.csv file in the Google Drive folder. I have pulled the codes and code meanings from the American Community Survey API and paired them in this document.
...
Next, we will use four `filter` operations (from the `dplyr` library) to find the number of people employed in each sector of interest. A comparison we can use in `filter` is `%in%`. When this logical expression is used, `filter` will keep any row with a value in the vector following `%in%`.
...
my_county_ag_employed <- filter(my_county_employ,
...
**\textcolor{orange}{(`r cont_count()`)}** Calculate the number of people that moved to your county between 2013-2017 (hint: use one of the columns from step (`r i-1`) in a function that will add all the values together). Save this value in an object called `inflow`.

**\textcolor{orange}{(`r cont_count()`)}** Calculate the number of people that moved out of your county between 2013-2017 (hint: use the other column from step (`r i-2`)). Save this value in an object called `outflow`.

```{r}
#| include: false

inflow <- sum(my_county_migration$Flow.from.Geography.B.to.Geography.A, na.rm=TRUE)
outflow <- sum(my_county_migration$Counterflow.from.Geography.A.to.Geography.B1, na.rm=TRUE)
```

**\textcolor{orange}{(`r cont_count()`)}** Print `inflow` and `outflow` to check your work.
...
\begin{math} netmigration  = \frac{inflow - outflow}{totalpopulation2015} \cdot 1000 \end{math}

```{r}
#| include: false

net_migration <- (inflow-outflow)/total_population_2015 * 1000
```
