---
title: "Integrating Migration Data - Answer Key"
format: pdf
---
...
migration_filt <- subset(migration, 
                         County.of.Current.Residence %in% c("Ada County", 
                                                            "Canyon County", 
                                                            "Elmore County",
                                                            "Payette County", 
                                                            "Gem County", 
...
```{r}
migration_filt$pop_diff <-
  migration_filt$County.of.Current.Residence_Population.1.Year.and.Over_Estimate - 
  migration_filt$County.of.Residence.1.Year.Ago_Population.1.Year.and.Over_Estimate
```
...
```{r}
migration_filt$pop_diff_perc <- migration_filt$pop_diff /
  migration_filt$County.of.Residence.1.Year.Ago_Population.1.Year.and.Over_Estimate * 100
```
...
tot_movers <- summarise(migration_filt,
                        ...
                        total_movers = sum(Movers.in.County.to.County.Flow_Estimate, 
                                           na.rm=TRUE))
print(tot_movers)
```
...
```{r}
migration_filt_areas$curr_pop_density <-
  migration_filt_areas$County.of.Current.Residence_Population.1.Year.and.Over_Estimate /
  migration_filt_areas$curr_county_area_sq_mi

migration_filt_areas$prev_pop_density <-
  migration_filt_areas$County.of.Residence.1.Year.Ago_Population.1.Year.and.Over_Estimate /
  migration_filt_areas$prev_county_area_sq_mi
```

**\textcolor{orange}{(`r cont_count()`)}** Create a new column with the difference between the current and previous population density.

```{r}
migration_filt_areas$pop_dens_diff <- migration_filt_areas$curr_pop_density -
  migration_filt_areas$prev_pop_density
```
