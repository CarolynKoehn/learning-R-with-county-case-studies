...
We'll be using a a simplified data set of population demographics for each county in Idaho from the American Community Survey. **\textcolor{orange}{(`r cont_count()`)}** Download `ID_counties_tot_pop_med_age.csv` from this [GitHub data folder]() into the `data` folder you created in your project.

<!-- Instructors: you may use this link or specify how students should access data -->
...
**\textcolor{orange}{(`r cont_count()`)}** Download the file `producer_age_2017_ID.csv` from the Google Drive data folder into your `data` folder. 
...
You may notice that many of these columns have the same information in every row. This is a "dirty" dataset, and we will spend lots of time this semester learning how make data like this "clean."

**\textcolor{orange}{(`r cont_count()`)}** Make sure to identify the column that has the county names and check the format of the county names (hint: `County == "Bear Lake County, Idaho"` in the `subset` function will not work on this data).
...
Notice how I used `[]` with `head`, which is possible because `head` produces a new data frame. Since I knew the column names were similar to the column names from the other Census of Agriculture data, I chose to look at only a few interesting columns in `head` instead of all of them.
...
**\textcolor{orange}{(`r cont_count()`)}** Next, find the number of people in producer households for your county (Hint: refer to step `r subset_and_print_value`). This time, save the value to a new object before printing it.
```{r}
#| include: false
...
```{r}
# I made a subset of households called my_county_households here
...
Notice how the the number of people in producer households printed in quotation marks. `R` thinks this is a word, not a number, because there is a comma. If we try to do math now, `R` will give us an error because it doesn't know how to do math with a number and a word. **\textcolor{orange}{(`r cont_count()`)}** Try this in the Console (using your object names, if they differ from mine) and read the error message. 
...
In this message, `R` tells us that an error occurred (`Error`), where in the code it occurred (`in my_county_households_total/my_county_total_popuation`), and it's best desription of what went wrong (`non-numeric argument to binary operator`). The key information for us here is that there is a "non-numeric" object, even though we are trying to do math, which needs numbers.
...
**\textcolor{orange}{(`r cont_count()`)}** Download the file `cropland_pastureland_total_ares_ID_1997-2017.csv` from the Google Drive data folder into your `data` folder. 
