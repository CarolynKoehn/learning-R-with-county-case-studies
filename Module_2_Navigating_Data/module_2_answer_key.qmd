---
title: "Navigating data: Answer key"
format: pdf
---
...
```{r}
# fill in the blanks
producer_age <- read.csv("data/producer_age_2017_ID.csv")

# use at least two of these:
head(producer_age)
colnames(producer_age)
summary(producer_age)
```
...
```{r}
print(producer_age$County) # data_frame$column_name_of_county_column
```
...
```{r}
my_county_producer_age <- subset(producer_age, County == "BEAR LAKE")

print(my_county_producer_age$Value)
```
...
```{r}
round(perc_in_producer_households, digits = 1)
```
...
```{r}
unique(my_county_land$Commodity)
```
...
ag_land_comma_removed <- str_remove_all(string = my_county_ag_land[ ,"Value"], 
                                        pattern = ",")
my_county_ag_land_num <- as.numeric(ag_land_comma_removed)

total_land_comma_removed <- str_remove_all(string = my_county_total_land[ ,"Value"],
                             pattern = ",")
my_county_total_land_num <- as.numeric(total_land_comma_removed)
```
...
```{r}
print(my_county_ag_land_num)
print(my_county_total_land_num)
```
...
```{r}
perc_ag_land <- (sum(my_county_ag_land_num) / my_county_total_land_num) * 100
```
