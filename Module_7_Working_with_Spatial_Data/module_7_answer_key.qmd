---
title: "Working with Spatial Data - Answer Key"
format: pdf
---
...
Just like we made subsets in the last modules to narrow down the data to one county, we need to crop large rasters to a single county to subset spatial data. To do this, we're going to crop a raster of Idaho's land values to your county.
...
**\textcolor{orange}{(`r cont_count()`)}** Since vector data is a data frame, use `head` and/or `colnames` to find the column that holds the county names. 

```{r}
head(idaho)
```

**\textcolor{orange}{(`r cont_count()`)}** Create a `subset` of `idaho` where that column matches your county. This should create a data frame with one row.
...
```{r}
st_crs(bear_lake)$epsg

bear_lake_proj <- st_transform(bear_lake, 
                               crs = st_crs(land_value))

st_crs(bear_lake_proj)$epsg
```

**\textcolor{orange}{(`r cont_count()`)}** Plot the `st_geometry` of your projected county to check that it looks right.

```{r}
plot(st_geometry(bear_lake_proj))
```
...
```{r}
median(values(land_value_mask_trans), na.rm=TRUE)

quantile(values(land_value_mask_trans), na.rm=TRUE)
```
